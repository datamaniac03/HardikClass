...
update(diamondgraph, aspect="fill", layout=c(5,1))
update(diamondgraph, panel=panel.barchart)
update(diamondgraph, col="tomato") 

# 2.6 shingles
diamonds$shingle <- equal.count(price, number=10, overlap=0)
...
ggplot(diamonds, aes(clarity)) +facet_grid(.~cut) + geom_bar(color="SteelBlue",position="dodge", fill="black")+theme(panel.background = element_rect(fill='pink', colour='green'))
